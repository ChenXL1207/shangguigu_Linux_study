...
int main(int argc, char const *argv[])
{
    ...
    handle_error("socket",server_socketfd);
    // 2.绑定地址 bind
    int bind_result = bind(server_socketfd,(struct sockaddr *)&server_addr,sizeof(server_addr));
    if(bind_result < 0){
        perror("bind");
        exit(EXIT_FAILURE);
    }
    ...
    // 4.接受连接 accept,相当于建立一个新的socket链接
    socklen_t client_len = sizeof(client_addr);
    int client_socketfd = accept(server_socketfd,(struct sockaddr *)&client_addr,&client_len);
    handle_error("accept",client_socketfd);

    printf("与客户端%s %d建立链接 文件描述符是%d\n",inet_ntoa(client_addr.sin_addr),ntohs(client_addr.sin_port),client_socketfd);
    
    pthread_t pid_read;
    pthread_t pid_write;
    // 创建子线程用于收消息
    pthread_create(&pid_read,NULL,read_from_client,&client_socketfd);
    // 创建子线程用于发消息
    pthread_create(&pid_write,NULL,write_to_client,&client_socketfd);
    // 主线程等待子线程结束

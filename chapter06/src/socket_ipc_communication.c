...
void client_func(int socketfd)
{
    ...
    printf("启动客户端\n");
    // 1.连接服务器
    int result = connect(socketfd,(struct sockaddr *)&socket_addr,sizeof(socket_addr));
    handle_error("connect",result);
    // 2.发送消息
    strcpy(buffer,"Hello,Server!");
    result = send(socketfd,buffer,strlen(buffer),0);// 发送消息给服务器,buffer-->发送消息的缓冲区,strlen(message)-->发送消息的长度,0-->默认标志
    handle_error("send",result);
    // 3.接收消息
    memset(buffer,0,BUFFER_SIZE);
    result = recv(socketfd,buffer,BUFFER_SIZE,0);// 接收消息,buffer-->接收消息的缓冲区,BUFFER_SIZE-->接收消息的最大长度,0-->默认标志
    handle_error("recv",result);
    printf("接收到服务器消息: %s\n",buffer);
    // 4.发送EOF
    strcpy(buffer,"EOF");
    result = send(socketfd,buffer,strlen(buffer),0);// 发送EOF给服务器,buffer-->发送EOF的缓冲区,strlen(message)-->发送EOF的长度,0-->默认标志
    handle_error("send",result);
    // 5.接收EOF
    memset(buffer,0,BUFFER_SIZE);
    result = recv(socketfd,buffer,BUFFER_SIZE,0);// 接收EOF,buffer-->接收EOF的缓冲区,BUFFER_SIZE-->接收EOF的最大长度,0-->默认标志
    handle_error("recv",result);
    printf("接收到服务器EOF: %s\n",buffer);
}